...
$!
$! P1 specifies the config.h.in file or equivalent or the keyword "NOBUILTINS".
$! P2 may also be the keyword "NOBUILTINS" if P1 specifies a file.
$! P2 or P3 can be "VERBOSE".
$!
$! If P1 does not specify a file then this procedure will search for
$! several common names of the file.
$!
...
$! 17-Aug-2013	J. Malmberg	Update for coreutils
$! 12-Jun-2016	J. Malmberg	Merge in Bash changes
$!============================================================================
$!
$ss_normal = 1
$ss_abort = 44
...
$!
$nobuiltins = ""
$if (p1 .eqs. "NOBUILTINS") .or. (p2 .eqs. "NOBUILTINS")
$then
$   nobuiltins = "NOBUILTINS"
$endif
$!
$verbose = ""
$if (p2 .eqs. "VERBOSE") .or. (p3 .eqs. "VERBOSE")
$then
$   verbose = "VERBOSE"
$endif
$!
...
$!------------------------------------------------------------------
$if p1 .nes. "" .and. p1 .nes. "NOBUILTINS"
$then
$   cfile = p1
$else
...
$   endif
$   xlen = f$length(xline)
$   line_end = f$extract(xlen - 1, 1, xline)
$   if line_end .eqs. "\" then cont_line = 1
$   comment_end = f$extract(xlen - 2, 2, xline)
...
$   then
$	xline_end = ""
$	if comment_end .nes. "*/" then xline_end = " */"
$	write tf "/* ", xline, xline_end
...
$   endif
$!
$!  Handle #define specials
$!---------------------------
$   if key1 .eqs. "#define"
$   then
$	if cont_line .eq. 0
$	then
$	    if key2 .eqs. "_GL_INLINE"
$	    then
$		write tf "#define ''key2' static inline"
$		goto cfgh_in_loop1
$	    endif
$	    if f$locate("_GL_INLINE", xline) .lt xlen
$	    then
$		write tf xline
$		goto cfgh_in_loop1
$	    endif
$	    if f$locate("_GL_EXTERN_INLINE", xline) .lt xlen
$	    then
$		write tf xline
$		goto cfgh_in_loop1
$	    endif
$	    if f$locate("_GL_CMP", xline) .lt xlen
$	    then
$		write tf xline
$		goto cfgh_in_loop1
$	    endif
...
$	if x .lt. xlen
$	then
$	    xline_start = f$extract(0, x, xline)
$	    xline_end = f$extract(x, xlen - x, xline)
...
$	then
$	    write tf "#ifndef ''key2'"
$	    write tf "#define ''key2' 4294967295u"
$	    write tf "#endif"
$	    goto cfgh_in_loop1
$	endif
$!
$	if key2 .eqs. "SSIZE_MAX"
$	then
$	    write tf "#ifndef ''key2'"
$	    write tf "#define ''key2' 4294967295u"
$	    write tf "#endif"
...
$	endif
$!
$	if (key2 .eqs. "HAVE_ISNAND_IN_LIBC") .or. -
	   (key2 .eqs. "HAVE_ISNANF_IN_LIBC") .or. -
	   (key2 .eqs. "HAVE_ISNANL_IN_LIBC")
$	then
$	    write tf "#ifndef ''key2'"
$	    write tf "#define ''key2' 1"
$	    write tf "#endif"
...
$	endif
$!
$	if (key2 .eqs. "HAVE_LDEXPL_IN_LIBC") .or. -
	   (key2 .eqs. "HAVE_LDEXP_IN_LIBC")
$	then
$	    write tf "#ifndef ''key2'"
$	    write tf "#define ''key2' 1"
$	    write tf "#endif"
...
$	endif
$!
$	if key2 .eqs. "HAVE_LONG_FILE_NAMES"
$	then
$	    write tf "#ifndef ''key2'"
$	    write tf "#define ''key2' 1"
$	    write tf "#endif"
...
$	endif
$!
$	if key2 .eqs. "HAVE_MINIMALLY_WORKING_GETCWD"
$	then
$	    write tf "#ifndef ''key2'"
$	    write tf "#define ''key2' 1"
$	    write tf "#endif"
...
$	endif
$!
$	if (key2 .eqs. "HAVE_SNPRINTF_RETVAL_C99") .or. -
	   (key2 .eqs. "HAVE_SNPRINTF_TRUNCATION_C99")
$	then
$	    write tf "#ifndef ''key2'"
$	    write tf "#define ''key2' 1"
$	    write tf "#endif"
...
$	endif
$!
$	if (key2 .eqs. "HAVE_STRUCT_TIMEVAL") .or. -
	   (key2 .eqs. "HAVE_STRUCT_UTIMBUF")
$	then
$	    write tf "#ifndef ''key2'"
$	    write tf "#define ''key2' 1"
$	    write tf "#endif"
...
$	endif
$!
$	if key2 .eqs. "HAVE_WORKING_VFORK"
$	then
$	    write tf "#ifndef ''key2'"
$	    write tf "#define ''key2' 1"
$	    write tf "#endif"
...
$	endif
$!
$	if key2 .eqs. "NEED_MKTIME_INTERNAL"
$	then
$	    write tf "#ifndef ''key2'"
$	    write tf "#define ''key2' 1"
$	    write tf "#endif"
...
$	endif
$!
$	if f$locate("GNULIB_", key2) .lt. key2_len
$	then
$	    write tf "#ifndef ''key2'"
$	    write tf "#define ''key2' 1"
$	    write tf "#endif"
...
$		    write tf "#ifndef ''key2'"
$		    write tf "#define ''key2' 1"
$if verbose .nes. "" then write sys$output "''dchfile' - #define ''key2' 1"
$		    write tf "#endif"
...
$		    write tf "#ifndef ''key2'"
$		    write tf "#define ''key2' 1"
$if verbose .nes. "" then write sys$output "''starfile' - #define ''key2' 1"
$		    write tf "#endif"
$		    if key64 then write tf "#endif"
$		    goto cfgh_in_loop1
$		endif
...
$		    endif
$!
$		    if (key2c .eqs. "STATVFS") .and. key2d .nes. ""
$		    then
$!		        Should really verify that the structure
$!		        named by key2c actually exists first.
$!------------------------------------------------------------
$!
$!		        Statvfs structure members
$!-------------------------------------------------
$		        key2de = key2d + "_" + key2e
$		        if key2d .eqs. "F"
$			then
$			    keysym = f$edit(key2de, "LOWERCASE")
$			else
$			    keysym = "f_" + f$edit(key2d,"LOWERCASE")
$			endif
$		    endif
$		endif
...
$			if key2c .eqs. "F"
$			then
$			    keysym = f$edit(key2cd, "LOWERCASE")
$			else
...
$		endif
$!
$		if (keysym .eqs. "ERRNO_DECL") .or. -
		   (keysym .eqs. "HAVE_DECL_ALARM") .or. -
		   (keysym .eqs. "HAVE_DECL_FREEADDRINFO") .or. -
		   (keysym .eqs. "HAVE_DECL_FSEEKO") .or. -
		   (keysym .eqs. "HAVE_DECL_FTELLO") .or. -
		   (keysym .eqs. "HAVE_DECL_FLOCKFILE") .or. -
		   (keysym .eqs. "HAVE_DECL_FUNLOCKFILE") .or. -
		   (keysym .eqs. "HAVE_DECL_GAI_STRERROR") .or. -
		   (keysym .eqs. "HAVE_DECL_GETADDRINFO") .or. -
		   (keysym .eqs. "HAVE_DECL_GETCWD")
$		then
$		    write tf "#ifndef ''keysym'"
$		    write tf "#define ''keysym' 1"
$		    write tf "#endif"
$		    goto cfgh_in_loop1
$		endif
$!
$		if (keysym .eqs. "HAVE_DECL_GETENV") .or. -
		   (keysym .eqs. "HAVE_DECL_GETEUID") .or. -
		   (keysym .eqs. "HAVE_DECL_GETGRGID") .or. -
		   (keysym .eqs. "HAVE_DECL_GETLOGIN") .or. -
		   (keysym .eqs. "HAVE_DECL_GETNAMEINFO") .or. -
		   (keysym .eqs. "HAVE_DECL_GETPWUID") .or. -
		   (keysym .eqs. "HAVE_DECL_GETUID") .or. -
		   (keysym .eqs. "HAVE_DECL_INET_NTOP") .or. -
		   (keysym .eqs. "HAVE_DECL_INET_PTON") .or. -
		   (keysym .eqs. "HAVE_DECL_LOCALTIME_R")
$		then
$		    write tf "#ifndef ''keysym'"
$		    write tf "#define ''keysym' 1"
$		    write tf "#endif"
...
$		endif
$!
$		if (keysym .eqs. "HAVE_DECL_MBRLEN") .or. -
		   (keysym .eqs. "HAVE_DECL_MBSINIT") .or. -
		   (keysym .eqs. "HAVE_DECL_MBSRTOWCS") .or. -
		   (keysym .eqs. "HAVE_DECL_MEMCHR") .or. -
		   (keysym .eqs. "HAVE_DECL_SETENV") .or. -
		   (keysym .eqs. "HAVE_DECL_SLEEP") .or. -
		   (keysym .eqs. "HAVE_DECL_SNPRINTF") .or. -
		   (keysym .eqs. "HAVE_DECL_STRDUP") .or. -
		   (keysym .eqs. "HAVE_DECL_STRNLEN") .or. -
		   (keysym .eqs. "HAVE_DECL_TOLOWER")
$		then
$		    write tf "#ifndef ''keysym'"
$		    write tf "#define ''keysym' 1"
$		    write tf "#endif"
...
$		endif
$!
$		if (keysym .eqs. "HAVE_DECL_TZNAME") .or. -
		   (keysym .eqs. "HAVE_DECL_UNSETENV") .or. -
		   (keysym .eqs. "HAVE_DECL_WCRTOMB") .or. -
		   (keysym .eqs. "HAVE_DECL_WCTOB") .or. -
		   (keysym .eqs. "HAVE_DECL_WCWIDTH") .or. -
		   (keysym .eqs. "HAVE_DECL_SLEEP") .or. -
		   (keysym .eqs. "HAVE_DECL_SNPRINTF") .or. -
		   (keysym .eqs. "HAVE_DECL_STRDUP") .or. -
		   (keysym .eqs. "HAVE_DECL_STRNLEN") .or. -
...
$		endif
$!
$		if (keysym .eqs. "HAVE_DECL_TTYNAME")
$		then
$		    write tf "#ifndef ''keysym'"
$		    write tf "#define ''keysym' 1"
$		    write tf "#define __POSIX_TTYNAME 1"
$		    write tf "#endif"
$		    goto cfgh_in_loop1
$		endif
$!
...
$!-----------------------------------------------
$		if (keysym .eqs. "UNSIGNED_LONG_LONG_INT") .or. -
		   (keysym .eqs. "UNSIGNED_LONG_LONG") .or. -
		   (keysym .eqs. "LONGLONG") .or. -
		   (keysym .eqs. "LONG_LONG") .or. -
		   (keysym .eqs. "LONG_LONG_INT") .or. -
		   (keysym .eqs. "LONG_DOUBLE")
$		then
$		    write tf "#ifndef __VAX"
...
$		    write tf "#ifndef HAVE_''keysym'"
$		    write tf "#define HAVE_''keysym' 1"
$if verbose .nes. "" then write sys$output "''decc_shr' #define ''keysym' 1"
$		    write tf "#endif
$		    goto cfgh_in_loop1
$		endif
$!
...
$			write tf "#ifndef ''key2'"
$			write tf "#define ''key2' 1"
$if verbose .nes. "" then write sys$output "''decc_shr' #define ''key2' 1"
$			write tf "#endif"
$			if key64 then write tf "#endif"
$			goto cfgh_in_loop1
$		    endif
...
$		    write tf "#ifndef ''key2'"
$		    write tf "#define ''key2' 1"
$if verbose .nes. "" then write sys$output "''decc_rtldef' #define ''key2' 1"
$		    write tf "#endif"
$		    if key64 then write tf "#endif"
$		    goto cfgh_in_loop1
$		endif
...
$	endif
$!
$!      Process BITSIZEOF directives found in coreutils
$!--------------------------------------------------------
$	if key2a .eqs. "BITSIZEOF"
$	then
$	    if key2b .eqs. "PTRDIFF" .and. key2_h .eqs. "_T"
$	    then
$		write tf "#ifndef BITSIZEOF_PTRDIFF_T"
$		write tf "#if __INITIAL_POINTER_SIZE == 64"
$		write tf "#define BITSIZEOF_PTRDIFF_T 64"
$		write tf "#else"
$		write tf "#define BITSIZEOF_PTRDIFF_T 32"
$		write tf "#endif"
$		write tf "#endif"
$		goto cfgh_in_loop1
$	    endif
$	    if key2b .eqs. "SIG" .and. key2c .eqs. "ATOMIC" .and. -
		key2_h .eqs. "_T"
$	    then
$		write tf "#ifndef BITSIZEOF_SIG_ATOMIC_T"
$		write tf "#define BITSIZEOF_SIG_ATOMIC_T 32"
$		write tf "#endif"
$		goto cfgh_in_loop1
$	    endif
$	    if key2b .eqs. "SIZE" .and. key2_h .eqs. "_T"
$	    then
$		write tf "#ifndef BITSIZEOF_SIZE_T"
$		write tf "#define BITSIZEOF_SIZE_T 32"
$		write tf "#endif"
$		goto cfgh_in_loop1
$	    endif
$	    if key2b .eqs. "WCHAR" .and. key2_h .eqs. "_T"
$	    then
$		write tf "#ifndef BITSIZEOF_WCHAR_T"
$		write tf "#define BITSIZEOF_WCHAR_T 32"
$		write tf "#endif"
$		goto cfgh_in_loop1
$	    endif
$	    if key2b .eqs. "WINT" .and. key2_h .eqs. "_T"
$	    then
$		write tf "#ifndef BITSIZEOF_WINT_T"
$		write tf "#define BITSIZEOF_WINT_T 32"
$		write tf "#endif"
$		goto cfgh_in_loop1
$	    endif
$	endif
...
$	endif
$!
$!	Process FLEXIBLE_ARRAY_MEMBER directive if not C99 set to 1
$!----------------------------------
$	if key2 .eqs. "FLEXIBLE_ARRAY_MEMBER"
$	then
$	    write tf "#ifndef ''key2'"
$	    write tf "#define ''key2' 1"
$	    write tf "#endif"
...
$	endif
$!
$!	Process PROMOTED_MODE_T directive
$!----------------------------------
$	if key2 .eqs. "PROMOTED_MODE_T"
$	then
$	    write tf "#ifndef ''key2'"
$	    write tf "#define ''key2' int"
$	    write tf "#endif"
