...
int vms_open(const char *file_spec, int flags, ...) {
    ...
    if ((flags & ~mask) == 0) {
        int st_result;
        struct stat st_buf;
        st_result = stat(file_spec, &st_buf);
        if ((st_result == 0) &&  S_ISDIR(st_buf.st_mode)) {
            ...
            if (fd_result >= 0) {
                ...
                info->vmscwd = getcwd(NULL, (MAX_DIR_PATH + 1), 1);
                if (info->vmscwd == NULL) {
                    return -1;
                }
/* debug */
                dump_pointer_m(info->vmscwd, "vms_open/getcwd",
                               (MAX_DIR_PATH + 1));
...
char * vms_crtl_searchlist_getcwd_hack(void) {
    ...
    original_cwd = getcwd(NULL, MAX_DIR_PATH + 1, 1);
